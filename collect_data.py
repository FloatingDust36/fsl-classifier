...
def main():
    ...
    with HandLandmarker.create_from_options(options) as detector:
        ...
        while cap.isOpened():
            ...
            status = f"Letter: {label} | Samples: {samples_collected}/{SAMPLES_PER_SESSION}"
            if recording:
                status += " | RECORDING"
            cv2.putText(frame, status, (10, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

            cv2.imshow('FSL Data Collection', frame)
            ...
            if samples_collected >= SAMPLES_PER_SESSION:
                print(
                    f"Done! Collected {samples_collected} samples for '{label}'")
                break
